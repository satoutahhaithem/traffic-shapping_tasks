#!/usr/bin/env python3
"""
Traffic Control Performance Comparison with Synchronized Settings

This script measures the performance of video streaming under different network
conditions. It gets the commanded traffic control settings from the tc_settings_receiver
and compares them with the actual measured performance.

Usage:
    python tc_performance_sync.py [--sender-ip SENDER_IP] [--receiver-ip RECEIVER_IP]
...
# Import tc_settings_receiver module
def import_tc_settings_receiver():
    """Import the tc_settings_receiver module to get current settings"""
    try:
        # Get the path to the tc_settings_receiver.py file
        script_dir = os.path.dirname(os.path.abspath(__file__))
        module_path = os.path.join(script_dir, "tc_settings_receiver.py")
        
        # Import the module
        spec = importlib.util.spec_from_file_location("tc_settings_receiver", module_path)
        tc_settings_receiver = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(tc_settings_receiver)
        
        return tc_settings_receiver
    except Exception as e:
        print(f"{Colors.RED}Error importing tc_settings_receiver module: {e}{Colors.ENDC}")
        print(f"{Colors.RED}Make sure tc_settings_receiver.py is in the same directory{Colors.ENDC}")
        return None
...
# Function to get current traffic control settings from tc_settings_receiver
def get_tc_settings(tc_settings_receiver):
    """Get the current traffic control settings from tc_settings_receiver"""
    if tc_settings_receiver:
        try:
            settings = tc_settings_receiver.get_current_settings()
            return settings
        except Exception as e:
            print(f"{Colors.RED}Error getting traffic control settings: {e}{Colors.ENDC}")
    
    # Return default values if tc_settings_receiver is not available
    return {
        "preset": "UNKNOWN",
        "rate": 0,
        "delay": 0,
...
def collect_metrics(sender_ip, sender_port, receiver_ip, receiver_port, interval, duration, tc_settings_receiver):
    global running, data
    ...
    try:
        while running and (duration <= 0 or time.time() - start_time < duration):
            ...
            tc_settings = get_tc_settings(tc_settings_receiver)
            
            # Store commanded values
            data["commanded"]["rate"].append(tc_settings["rate"])
            data["commanded"]["delay"].append(tc_settings["delay"])
            data["commanded"]["loss"].append(tc_settings["loss"])
            ...
            if count % 5 == 0:
                ...
                print(f"{Colors.CYAN}Commanded Network Conditions: {tc_settings['preset']}{Colors.ENDC}")
                print(f"  Rate: {tc_settings['rate']} Mbps")
                print(f"  Delay: {tc_settings['delay']} ms")
                print(f"  Loss: {tc_settings['loss']}%")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Traffic Control Performance Comparison with Synchronized Settings")
    ...
    print(f"{Colors.HEADER}Traffic Control Performance Comparison with Synchronized Settings{Colors.ENDC}")
    ...
    # Import tc_settings_receiver module
    tc_settings_receiver = import_tc_settings_receiver()
    if not tc_settings_receiver:
        print(f"{Colors.RED}Error: Could not import tc_settings_receiver module{Colors.ENDC}")
        print(f"{Colors.RED}Make sure tc_settings_receiver.py is running and in the same directory{Colors.ENDC}")
        exit(1)
    ...
    # Check if we can get traffic control settings
    print(f"\n{Colors.CYAN}Checking traffic control settings...{Colors.ENDC}")
    
    tc_settings = get_tc_settings(tc_settings_receiver)
    if tc_settings["timestamp"] > 0:
        print(f"{Colors.GREEN}Successfully got traffic control settings{Colors.ENDC}")
        print(f"  Preset: {tc_settings['preset']}")
        print(f"  Rate: {tc_settings['rate']} Mbps")
        print(f"  Delay: {tc_settings['delay']} ms")
        print(f"  Loss: {tc_settings['loss']}%")
    else:
        print(f"{Colors.YELLOW}Warning: No traffic control settings received yet{Colors.ENDC}")
        print(f"{Colors.YELLOW}Make sure auto_tc_control_sync.sh is running on the sender{Colors.ENDC}")
    
    # Start metrics collection
    print(f"\n{Colors.CYAN}Starting metrics collection...{Colors.ENDC}")
    
    # Start collection in the main thread
    collect_metrics(args.sender_ip, args.sender_port, args.receiver_ip, args.receiver_port, args.interval, args.duration, tc_settings_receiver)
