...
if [ -f "dynamic_qos_streaming/video_streamer.py" ]; then
    echo "Updating receiver_ip in video_streamer.py to $RECEIVER_IP..."
    sed -i "s/receiver_ip = \"[0-9.]*\"/receiver_ip = \"$RECEIVER_IP\"/" dynamic_qos_streaming/video_streamer.py
    echo "Done!"
fi

# Update the interface in dynamic_tc_control.sh
if [ -f "dynamic_qos_streaming/dynamic_tc_control.sh" ]; then
    echo "Updating network interface in dynamic_tc_control.sh to $WIFI_INTERFACE..."
    sed -i "s/INTERFACE=\"[^\"]*\"/INTERFACE=\"$WIFI_INTERFACE\"/" dynamic_qos_streaming/dynamic_tc_control.sh
    echo "Done!"
fi
...
echo "  python3 dynamic_qos_streaming/video_streamer.py"
echo ""
echo "To apply traffic control (network conditions):"
echo "  sudo bash dynamic_qos_streaming/dynamic_tc_control.sh"
echo ""
